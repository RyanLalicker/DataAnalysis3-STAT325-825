...
Ordinal logistic regression models are expressed in terms of a logit function containing probabilities rather than a single variable.  In this case, where $Y$ is a random variable representing the calving ease score and $l=1,2,...,5$ represents the possible values of $Y$, we can say

$$
\ln\left(\frac{P(Y \leq l)}{P(Y > l)}\right)=
\alpha_l-\beta_{trt}X_{trt}-\beta_{sex}X_{sex}
$$
![Equation for calving ease ordinal logistic regression model.]

In the equation above the left side is the log-odds, which is the logit function mentioned previously.  On the right side of the equation $\alpha_l$ is the intercept of the model at the $l$th level, $X_{trt}$ and $X_{sex}$ represent the values for the third trimester treatment and the sex respectively, and the $\beta_i$ are slope values corresponding to the $i$th explanatory variable.

After fitting this model in R a significant difference between the DDG and CON was found.
