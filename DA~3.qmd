...
We have concerns with this approach however since this approach is designed to model nominal data, which is categorical variables without an order.
(@Multi).
For that reason we chose to go in another direction.
...
For this model, we chose to use the same two explanatory variables.
That means the equation can be written just as it was in the ordinal logistic regression, with the exception of $l$ which now only has the levels *low* and *high*.
